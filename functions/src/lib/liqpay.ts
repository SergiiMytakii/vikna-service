...
function normalizeInstallmentCount(
  ...
  if (paymentMethod !== "moment_part") {
    return undefined;
  }

  if (installmentCountRaw === undefined || installmentCountRaw === null) {
    return undefined;
  }

  const installmentCount = Number(installmentCountRaw);
  if (!Number.isInteger(installmentCount)) {
    throw new Error("Кількість платежів має бути цілим числом від 2 до 24");
  }

  if (installmentCount < 2 || installmentCount > 24) {
    throw new Error("Кількість платежів має бути в діапазоні від 2 до 24");
  }
